...
export async function createLead(req: Request, res: Response): Promise<void> {
  ...
  if (hasResult) {
    const attempt = {
      ...
      totalQuestions:
        totalQuestions != null ? Number(totalQuestions) : undefined,
    ...
    const attemptNumber = lead.attempts.length;
    const shouldSendEmail = attemptNumber === 1;

    res.status(201).json({
      success: true,
      message: shouldSendEmail
        ? "Lead saved. Report email will be sent shortly."
        : "Lead saved. Report email sent only for first attempt (skipped to conserve quota).",
      id: lead._id,
      attemptNumber,
